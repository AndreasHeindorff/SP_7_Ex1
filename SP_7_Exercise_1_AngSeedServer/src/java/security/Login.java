...
public class Login {
    ...
    public Response login(String jsonString) throws JOSEException, NoSuchAlgorithmException, InvalidKeySpecException {
        UserFacade fac = new UserFacade();
        JsonObject json = new JsonParser().parse(jsonString).getAsJsonObject();
        String username = json.get("username").getAsString();
        String password = json.get("password").getAsString();
        UserEntity user = fac.getUserByUsername(username);

        JsonObject responseJson = new JsonObject();
        List<String> roles;

        if ((roles = authenticate(username, password)) != null && PasswordHash.validatePassword(password, user.getPassword())) {
